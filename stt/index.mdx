...
<CardGroup cols={2}>
  ...
  <Card title="Word timestamps" icon="clock" href="/stt/timestamps">
    Every word with a start and end. One flag, no extra cost, subtitles included.
  </Card>
</CardGroup>
...
Need to know *when* each word was said? Add `?timestamps=true` and you also get `words` and `segments` with start/end times — see [word-level timestamps](/stt/timestamps).
...
<CardGroup cols={2}>
  <Card title="Live streaming" icon="tower-broadcast" href="/stt/streaming">
    Transcribe over a WebSocket while the speaker is still talking.
  </Card>
