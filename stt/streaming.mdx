...
Up to 200 terms, applied to finals. `ready` (or the config ack) reports `context_terms`
accepted and names any `context_skipped` — terms the model's 70-character alphabet cannot
express, like digits ("DX5" → send "дх five"-style spellings, or the spoken form). Biasing
is subtractive: a half-matched term earns nothing, so an irrelevant term list measurably
does not bend transcripts (that invariant is part of our release gates).
