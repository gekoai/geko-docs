...
The service scales to zero, so **the first connection after an idle period waits ~15–25 seconds** while a container boots and loads the model. Connections after that are fast — the first partial typically arrives ~1.5s in.
